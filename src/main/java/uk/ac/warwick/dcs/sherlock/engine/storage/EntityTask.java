...
public class EntityTask implements ITask, IStorable, Serializable {
	...
	public Class<? extends IDetector> getDetector() {
		try {
			return (Class<? extends IDetector>) Class.forName(this.detector, true, SherlockEngine.classloader).asSubclass(IDetector.class);
		}
		catch (Exception e) {
	...
	void clearResults() {
		...
		// Clear hash to indicate no results stored
		this.hash = null;
		
		// Reset status to PREPARED so it will run preprocessing and matching again
		this.status = WorkStatus.PREPARED;
